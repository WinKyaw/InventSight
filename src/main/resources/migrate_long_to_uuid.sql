...
-- Step 6: Remove old foreign key constraints
ALTER TABLE user_store_roles DROP CONSTRAINT IF EXISTS fk_user_store_roles_store_id;
ALTER TABLE sale_items DROP CONSTRAINT IF EXISTS fk_sale_items_product_id;
ALTER TABLE sales DROP CONSTRAINT IF EXISTS fk_sales_store_id;
-- Events: conditional drop
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'events' AND constraint_name = 'fk_events_store_id'
    ) THEN
        ALTER TABLE events DROP CONSTRAINT IF EXISTS fk_events_store_id;
    END IF;
END $$;

-- Step 7: Drop old primary key constraints
